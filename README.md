...
- 핵심 시스템
  ...
  => 전술 자원 시스템
  ==> Gold : 적군 처치 시, 혹은 Supporter 타워의 액션을 통해 Gold를 수급할 수 있음
  => 랜덤 타워와 타워 합성
  ==> 타워 건설 시, 랜덤한 타워가 생성되며 타워 등급과 타입이 같은 타워를 합성할 수 있고 합성 시, 더 높은 등급의 랜덤한 타워로 생성된다.
- 참여 인원 : 1인
...
- 블루프린트 인터페이스 (BPI) + EventDispatcher:
  => 참조없이 함수 실행만으로 클래스 간 의존성(Coupling)을 낮춥니다
